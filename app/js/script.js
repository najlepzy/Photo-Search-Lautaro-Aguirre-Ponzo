/* 
Creamos un objeto teniendo en cuenta los parámetros que nos devuelve la API en la documentación de la misma. 
...
const validateErrors = (data, searchValue) => {
  // Si no tiene más paginaciones, oculta el botón de cargar más fotos.
  if (!data.next_page) {
...
const getSearchedPhotos = async (e) => {
  ...
  if (validateSearchValue(searchValue) === true) {
    ...
    const data = await fetchPhotos(
      `https://api.pexels.com/v1/search?query=${searchValue}&per_page=16`
    );
...
const getMoreSearchedPhotos = async (index) => {
  // Realizamos el fetch de la petición.
  const data = await fetchPhotos(
    `https://api.pexels.com/v1/search?query=${searchValue}&per_page=16&page=${index}`
  );
  // Validamos los errores si los hay.
  validateErrors(data, searchValue);
  // Cargamos los nuevos items de la galería.
...
const getInitialRandomPhotos = async (index) => {
  // Realizamos el fetch de la petición.
  const data = await fetchPhotos(
    `https://api.pexels.com/v1/curated?per_page=16&page=${index}`
  );
